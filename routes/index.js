...
router.post('/loadasset', (req, res) => {
    var { ID } = req.body;
    query("SELECT storage, ext FROM public.data WHERE id=" + ID + ";", [], (err, rows) => {
        let storage = rows[0].storage
        let file = `video${rows[0].ext}`
        try {
            var process = new ffmpeg(`uploads/${storage}/${file}`);
                    ...
                    video.fnExtractFrameToJPG(tmp, {
                        ...
                        mergeImages(
                            ...
                            .then(b64 => {
                                ...
                                fs.writeFile(`uploads/${storage}/frame.jpg`, b64, { encoding: 'base64' }, function (err) {
                                    console.log(err);
                                });
                            });
